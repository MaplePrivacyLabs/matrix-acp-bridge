...
The running Matrix SDK client also serves Matrix MCP tools over a private Unix socket. ACP new/load/resume requests receive a stdio proxy server automatically. Search paginates server history and decrypts through that same device store; no second Matrix login is created. Explicit-send sessions receive a per-run connection capability outside the model-facing tool arguments. `send_message_to_thread` routes through the runner's single-writer outbox and waits for all Matrix acknowledgements; it cannot select another conversation. Sender capabilities expire at run completion. Ordinary ACP assistant text is discarded from the Matrix output path in explicit mode, while the agent retains its own session transcript.
