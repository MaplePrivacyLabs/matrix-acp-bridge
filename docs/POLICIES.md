...
| `operator_trust = "account"` | Known Matrix sender device plus operator allowlist; accepts devices the owner has not verified. Trusts the homeserver/account authentication, including administrators who can issue user sessions. No per-person DM/SAS verification. Wizard/example default. |
...
| `message_delivery = "explicit"` | Only `send_message_to_thread` calls publish agent-authored messages. Assistant text remains in the ACP session, with no fallback. Omission or `automatic` preserves text forwarding. Status reactions and required approval requests are independent. |
| `harness.mode` | Optional; when set, must be advertised and applied by the adapter; an unsolicited change stops the run. Tool sandbox/access behavior is adapter-specific. |
...
- **Encrypted rooms and HTTPS only.** Sender/device mismatch, unknown/insecure key origin, or a verification violation cannot authorize work. Plaintext and sender/key mismatches are rejected even in account-trust mode. This does not prevent a homeserver administrator from creating a valid but owner-unverified device for an approved account.
...
- **Steering.** Active-conversation messages use the configured non-interrupting strategy: native safe-point input where supported, otherwise continuation after the current turn. Follow-ups do not cancel tools. An end-of-turn race starts an immediate continuation, not a queued independent job. An optional positive worker-wide concurrency limit can still reject a new independent conversation; set it to 0 to remove that limit.
...
- **ACP compatibility.** Stable protocol v1 and load/resume are required. A configured mode must be advertised and applied; the mode may be omitted for agents without ACP modes. Agents must provide their own filesystem and terminal tools; the bridge does not expose client filesystem/terminal capabilities. No interactive provider login is performed in Matrix.
...
- **Output.** Text replies and status reactions are supported. Rich artifact upload is not. Replies remain encrypted and use standard Matrix threads. Streamed assistant-message text is combined until turn completion or a tool, permission or continuation boundary, rather than split by timer ticks. Reasoning events, tool calls/results and generated tool-progress notices are not published. In automatic delivery mode, agent-authored progress sent as ordinary assistant-message text is preserved: the bridge does not infer intent from wording, and ACP adapters may not distinguish commentary from final replies. Manual permission requests still appear when human input is required.
...
Explicit send tools are bound to the active run's conversation through a local connection capability; callers cannot supply a different room, thread or sender. Capabilities expire when the run finishes. Delivery still checks encryption, membership and current room policy through the ordinary durable outbox. A policy-blocked explicit send fails and is not sent later. An unconfirmed network send remains in the durable outbox; a lost tool response does not prove that the message was not sent. These capabilities prevent accidental cross-session routing; they do not isolate processes sharing the same OS account.
