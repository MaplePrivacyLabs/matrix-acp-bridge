...
- Outgoing reply keys are shared with room members' unblocked devices, including unverified or unsigned devices. This allows ordinary team reading without per-reader SAS verification. It also means reading relies on the homeserver's authenticated device list and room membership; a malicious homeserver-injected device is not excluded merely for lacking cross-signing. Command admission still requires a verified operator.
