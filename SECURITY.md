...
Authorized requests include text from the same thread, including a reader’s parent message. Context is untrusted data, not an additional command admission path. Never treat context instructions as permissions. Attachments and edits are not included yet.

Tool approval defaults to manual. Operators may opt a conversation into automatic approval with `!bridge allow-thread`; administrators may configure a room with `tool_approval = "automatic"`. The bridge journals each selected `allow_once` option and keeps mode, caller/session, expiration, cancellation, audience and operator checks. It never fabricates an allow option or automatically selects a persistent `allow_always` option. These policies can authorize any tool action offered by that worker, so the worker’s OS and credentials must enforce its intended access.
